...
class LOLApi:
    ...
    def __get_champion_data(self, champion_name):
        return self.champion_list[champion_name]

    def attributes(self, champion_name):
        tags = self.__get_champion_data(champion_name)['tags']
        return tags

    def partype(self, champion_name):
        partype = self.__get_champion_data(champion_name)['partype']
        if partype == 'Mana':
            return 'Normal'
        return 'Weird'
    ...
    def body_type(self, champion_name):
        ...
        armor = self.__get_champion_data(champion_name)['stats']['armor']
        spell_block = self.__get_champion_data(champion_name)['stats']['spellblock']
    ...
    def strength(self, champion_name):
        ...
        attack_damage = self.__get_champion_data(champion_name)['stats']['attackdamage']
        attack_damage_per_level = self.__get_champion_data(champion_name)['stats']['attackdamageperlevel']
        if attack_damage > 55 and attack_damage_per_level > 3:
            return 'Strong'
        return 'Kind'

    def get_champion_image(self, champion_name):
        champion = self.__get_champion_data(champion_name)
